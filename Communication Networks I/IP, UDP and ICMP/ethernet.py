...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    #print("\033[92m\n[DEBUG]: === process_Ethernet_frame === \033[0m") 
    
    destino = (data[0:6]) #bytes 0-5
    origen = (data[6:12]) #bytes 6-11
    etherType = struct.unpack("!H", data[12:14])[0] #bytes 12-13
    payload = bytes(data[14:])


    if(destino != macAddress and destino != broadcastAddr): # La trama no estaba destinada a este equipo
        #print(f"\033[92m\n[DEBUG][process_Ethernet_frame]: destino ({destino}) != macAddress ({macAddress})\033[0m") 
        return
    
    
    if(etherType not in EthernetProtocols): # No hay callback asociado al ethertype
        #print(f"\033[92m\n[DEBUG][process_Ethernet_frame]: ethertype INVÁLIDO\033[0m") 
        return
    
    #print(f"[DEBUG][process_Ethernet_frame]:destino:{destino}|mac adress:{macAddress}|broadcast:{broadcastAddr}\n")
    #print(f"\033[92m\n[DEBUG][process_Ethernet_frame]: recibida una trama PARA NOSOTROS. Ethertype: {etherType} \033[0m") 
    #print(f"\033[92m\n[DEBUG][process_Ethernet_frame]: Ethertype COOOOOOOOORRRRRRRREEEEECTO: {etherType} \033[0m") 
    
    callback = EthernetProtocols[etherType]
    callback(us, header, payload, origen)
...
def registerEthCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    #print("\033[92m\n[DEBUG]: === registerEthCallback === \033[0m") 

    #EthernetProtocols es el diccionario que relaciona ethertype y función de callback
    EthernetProtocols[ethertype] = callback_func
    #print(f"\033[95m\n[DEBUG]: ETHERNET PROTOCOLS: {EthernetProtocols} \033[0m") 
...
def startEthernetLevel(interface:str) -> int:
    ...
    #print("\033[92m\n[DEBUG]: === startEthernetLevel === \033[0m") 
...
def stopEthernetLevel()->int:
    ...
    #print("\033[92m\n[DEBUG]: === stopEthernetLevel === \033[0m") 
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    #print("\033[92m\n[DEBUG]: === sendEthernetFrame === \033[0m") 

    global macAddress, handle
    header_size = 14

    if (length + header_size > ETH_FRAME_MAX):  # La cabecera ocupa 14 bytes
        return -1
    
    EthernetFrame = bytearray()

    # Cabecera
    EthernetFrame += dstMac  # Direccion destino
    EthernetFrame += macAddress  # Direccion origen
    EthernetFrame += struct.pack("!H", etherType)  # Tipo ethernet

    # Datos
    EthernetFrame += data

    # Padding (añadir 0s si es necesario)
    if (length + header_size < ETH_FRAME_MIN):
        padding = ETH_FRAME_MIN - (length + header_size)
        EthernetFrame += (b'\x00'*padding)
        #EthernetFrame += struct.pack(f'!{padding}x') 

    sent_length = len(EthernetFrame)

    #DEBUG 
    #print(f"Enviando trama Ethernet de {sent_length} bytes: {hexlify(EthernetFrame)}")

    if sent_length == pcap_inject(handle, bytes(EthernetFrame), sent_length):
        #print("SE ENVIÓ BIEN\n")
        return 0
    else:
        return -1