...
def sendUDPDatagram(data,dstPort,dstIP):
    ...
    checksum = 0
    udp_datagram += struct.pack("!H", srcPort)
    udp_datagram += struct.pack("!H", dstPort)
    udp_datagram += struct.pack("!H", len(data) + UDP_HLEN)
    udp_datagram += struct.pack("H", checksum)
    udp_datagram += data

    #print("Sending udp datagram:", udp_datagram)
...
def initUDP():
    ...
    #print("Init udp, registering protocol")