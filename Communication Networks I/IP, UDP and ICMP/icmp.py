...
def process_ICMP_message(us,header,data,srcIp):
    ...
    type = struct.unpack('!B', icmp_header[0:1])[0]
    code = struct.unpack('!B', icmp_header[1:2])[0]
    checksum = struct.unpack('H', icmp_header[2:4])[0]
    identifier = struct.unpack('!H', icmp_header[4:6])[0]
    seqnum = struct.unpack('!H', icmp_header[6:8])[0]

    icmp_header = icmp_header[:2] + struct.pack('H', 0) + icmp_header[4:]

    if chksum(icmp_header + icmp_data) != checksum:
        return
    
    logging.debug("Tipo: %d, Código: %d", type, code)
    
    if (type == ICMP_ECHO_REQUEST_TYPE):
        sendICMPMessage(icmp_data, ICMP_ECHO_REPLY_TYPE, 0, identifier, seqnum, srcIp)
    elif (type == ICMP_ECHO_REPLY_TYPE):
        timeLock.acquire()
        rtt = time.time() - icmp_send_times[srcIp+identifier+seqnum]
        timeLock.release()
        print("RTT: ", rtt)
...
def sendICMPMessage(data,type,code,icmp_id,icmp_seqnum,dstIP):
    ...
    if(type is not ICMP_ECHO_REQUEST_TYPE and type is not ICMP_ECHO_REPLY_TYPE):
        return False
    # Construir la cabecera ICMP campo a campo
    icmp_header += struct.pack('!B', type)
    icmp_header += struct.pack('!B', code)
    icmp_header += struct.pack('H', 0)  # Checksum inicial a 0
    ...
    if (len(data+icmp_header) % 2 != 0):
        data += struct.pack('!B', 0)  
        
    checksum = chksum(icmp_header+data)
    icmp_header = icmp_header[:2] + struct.pack('H', checksum) + icmp_header[4:]
      
    
    sendIPDatagram(dstIP, icmp_header + data, ICMP_PROTO)
    return True
