...
Status depth_first_search(Graph *graph, Vertex* vf, Vertex* vt)
{
	int i;
	Stack* stack;
...
Status graph_breathSearch (Graph *g, long from_id, long to_id){
	...
	all_vertex = graph_get_all_vertex(g);
	...
	while (queue_isEmpty(queue)==FALSE && flag==0)
	{
		...
		else
		{
			if (vertex_getState(v0)==WHITE)
			{
				...
				{
					connectionsFromId = graph_getConnectionsFromId(g, vertex_getId(v0));


					if (vertex_getState(graph_get_vertex_from_id(g,connectionsFromId[i]))==WHITE)
					{

						vertice_aux = (Vertex*) vertex_copy(graph_get_vertex_from_id(g,connectionsFromId[i]));
						if (vertice_aux==NULL)
						{
							st = ERROR;
						}
