 ...
 */
{
    ...
    menu->out_win_choices =
            (char **) calloc(windows->rows_out_win*128, sizeof(char *));

    menu->out_win_message =
            (char **) calloc(windows->rows_out_win*128, sizeof(char *));

    for (i = 0; i < windows->rows_out_win*128; i++){
    
        (menu->out_win_choices)[i] =
                (char *) calloc(windows->cols_out_win, sizeof(char *));

        (menu->out_win_message)[i] =
                (char *) calloc(windows->cols_out_win, sizeof(char *));
 ...
 */
{
    ...
    (void) box(win, 0, 0);
    (void) mvwaddstr(win, 0, 2, title);
    for (i = menuitems_inf-1; i < menuitmes_sup; ++i) {
