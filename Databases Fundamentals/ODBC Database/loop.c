    ...
    while ((bool) TRUE) {
        ...
        switch (ch) {
            ...
            case 0x2B: /* + */
                ...
                if (item_index(auxItem) == SEARCH && focus == FOCUS_LEFT) {
                    ...
                    out_highlight = MAX(out_highlight - 1, 0+aux);
                    print_out(out_win, menus->out_win_choices, 1+aux,MIN(max_rows, n_out_choices)+aux,
            ...
            case 0x2D: /* - */
                if (item_index(auxItem) == SEARCH && focus == FOCUS_LEFT) {
                    ...
                    if (page==max_pages){
                        out_highlight = MIN(out_highlight + 1, lpn_rows+aux-1);
                    }
                    else{
                    out_highlight = MIN(out_highlight + 1, MIN(max_rows, n_out_choices)+aux-1);
                    }
                    print_out(out_win, menus->out_win_choices,1+aux, MIN(n_out_choices, max_rows)+aux,
                              out_highlight, windows->out_title);
                }
                break;
            ...
            case 9:
                ...
                if (focus == FOCUS_LEFT && n_out_choices==0){
                    /*focus = FOCUS_LEFT;*/
                    (void) snprintf(buffer, 128, "Sin resultados que mostrar %d", focus);
                }
                else if (focus==FOCUS_LEFT){
                     focus = FOCUS_RIGHT;
            ...
            case 127:
                ...
                break;
            case 338: /* Av Pág has been pressed */
            if (focus == FOCUS_RIGHT){
                    aux+=max_rows;
                    page=MIN(page+1,max_pages);

                    if(aux>=n_out_choices){
                        aux-=max_rows;
                    }
                (void) wclear(out_win);
                    out_highlight = aux;
                ...
                break;
            case 339: /* Re Pág has been pressed*/
                if(focus == FOCUS_RIGHT){
                    aux-=max_rows;
                    page=MAX(page-1,1);
                    if(aux<=0){
        ...
        if (choice != -1 && enterKey) /* User did a choice process it */
        {
            if (choice == QUIT)
                ...
                status = results_search(tmpStr1, tmpStr2,tmpStr3, &n_out_choices, & (menus->out_win_choices),&(menus->out_win_message),
                ...
                print_out(out_win, menus->out_win_choices, 1, MIN(max_rows, n_out_choices),
                    ...
                    if (n_out_choices==0){
                        page=0;
                        if (status==0){
                            (void)snprintf(buffer, 128, "arg1=%s, arg2=%s, arg3=%s. No hay resultados",  tmpStr1, tmpStr2, tmpStr3);
                        }
                        else if (status==-1){
                            (void)snprintf(buffer, 128, "arg1=%s, arg2=%s, arg3=%s. Error en la ejecución de la consulta",  tmpStr1, tmpStr2, tmpStr3);
                        }
                        else if (status==-2){
                            (void)snprintf(buffer, 128, "arg1=%s, arg2=%s, arg3=%s. Error en la conexión con la base de datos",  tmpStr1, tmpStr2, tmpStr3);
                        }
                        ...
                        lpn_rows=n_out_choices%max_rows;
                        if (lpn_rows==0){
                ...
                (void)snprintf(buffer, 128, "msg=%s", (menus->out_win_message)[out_highlight] );
