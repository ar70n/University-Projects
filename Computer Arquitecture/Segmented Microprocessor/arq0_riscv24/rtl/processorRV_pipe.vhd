...
----** Etapa EX - Execution in ALU **----
  ...
  Addr_Jalr      <= reg_RS1_FW + Imm_ext_EX;
  ...
  Alu_Op1    <= reg_RS1_FW when Ctrl_auiPc_EX  = '0' else PC_reg_EX;
  Alu_Op2    <= reg_RS2_FW when Ctrl_ALUSrc_EX = '0' else Imm_ext_EX;

  reg_RS1_FW   <= reg_RS1_EX when ForwardA = "00" else 
                  Alu_Res_ME when ForwardA = "01" else
                  result_WB;

  reg_RS2_FW   <= reg_RS2_EX when ForwardB = "00" else 
                  Alu_Res_ME when ForwardB = "01" else
                  result_WB;
